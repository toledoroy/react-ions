...
class ExampleCustomTaglist extends React.Component {
  constructor(props) {
    super(props)
  }
  ...
  getTagList = ({ tags, displayProp, onRemove }) => (
    <ul>
      {
        tags.map(tag => (
          <li key={tag.display}>{tag.display}</li>
        ))
      }
    </ul>
  )
  ...
  render() {
    ...
    return (
      <div>
        ...
        <MultiSelect optClass={style['custom-multi']} options={options} valueProp='value' displayProp='display' value={this.state.value} changeCallback={this.handleChange} placeholder="Select one or more items" tagList={this.getTagList}>
          <SelectField />
        </MultiSelect>
...
export default ExampleCustomTaglist
