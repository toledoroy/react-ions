...
const ButtonAnchor = (props) => {
  ...
  if (props.internal) {
    buttonAnchor = <Link to={props.path} size={props.size} disabled={props.disabled} className={btnAnchorClasses}>{props.children}</Link>
  } else {
    buttonAnchor = <a href={props.path} size={props.size} disabled={props.disabled} className={btnAnchorClasses}>{props.children}</a>
  }
  return buttonAnchor
}
